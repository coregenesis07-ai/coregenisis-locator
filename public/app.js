...
function footer(){
  return `<footer class="footer"><div class="container footer-grid">
    ...
    <div><h4>Legal</h4><a href="#/privacy">${state.lang==='es'?'Privacidad':'Privacy'}</a><a href="#/terms">${state.lang==='es'?'Términos':'Terms'}</a><a href="#/disclaimer">${state.lang==='es'?'Aviso legal':'Disclaimer'}</a><a href="#/refunds">${state.lang==='es'?'Reembolsos y cancelación':'Refunds & Cancellation'}</a><a href="#/copyright">${state.lang==='es'?'Derechos de autor':'Copyright'}</a></div>
  </div></footer>`;
}
